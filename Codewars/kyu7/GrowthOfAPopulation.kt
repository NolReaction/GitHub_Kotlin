...
The population regularly increases by
2 percent per year and moreover 50 new inhabitants per year
...
the function nb_year should return
n number of entire years needed to get a population greater or equal to p.
...
At the end of each year, the population count is an integer:
...
object GrowthOfAPopulation {
    ...
    fun nbYear(pp0:Int, percent:Double, aug:Int, p:Int):Int {
        var year = 0
        var temp_pp0 = pp0.toDouble()

        while (temp_pp0 < p) {
            temp_pp0 = temp_pp0 + (temp_pp0 * percent / 100).toInt() + aug
            year++
        }
