...
object SpeedControl {
    ...
    fun gps(s:Int, x:DoubleArray):Int {
        ...
        val speeds = x.mapIndexed { index, value ->
            if (index == 0) 0.0
            else 3600 * (value - x[index - 1]) / s
        }

        return speeds.maxOrNull()?.toInt() ?: 0

    }

    fun gps2(s:Int, x:DoubleArray) = x.toList()
